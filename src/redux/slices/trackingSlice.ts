...
export const trackShipment = createAsyncThunk(
  ...
  async (shipmentId: string, { rejectWithValue }) => {
    try {
      ...
      if (!token) {
        return rejectWithValue("Authentication required. Please log in to track your shipments.");
      }

      if (!userId) {
        return rejectWithValue("User information not found. Please log in again.");
      }
      ...
      if (data.error) {
        ...
        if (errorMessage.toLowerCase().includes("unauthorized") ||
          ...
          return rejectWithValue("You are not authorized to track this shipment. You can only track shipments that belong to your account.");
        }
      ...
      if (!data.tracking || data.tracking.length === 0) {
        return rejectWithValue("No tracking data found for this shipment. Please verify the shipment ID and ensure it belongs to your account.");
      }
      ...
      if (trackingData.ShipmentDetails) {
        ...
        if (!shipmentUserId || shipmentUserId.toString() !== userId.toString()) {
          ...
          console.warn(`SECURITY VIOLATION: User ${userId} attempted to access shipment ${shipmentId} owned by ${shipmentUserId || 'unknown'}`);
          ...
          return rejectWithValue("UNAUTHORIZED ACCESS: You can only track shipments that belong to your account. This security violation has been logged for audit purposes.");
        }
      } else {
        // If no shipment details, deny access - this is suspicious
        console.warn(`SECURITY WARNING: No shipment details found for shipment ${shipmentId} requested by user ${userId}`);
        return rejectWithValue("Access denied. Unable to verify shipment ownership. Please ensure the shipment ID is correct and belongs to your account.");
      }

      // ADDITIONAL SECURITY: Cross-validate with customer ID if available
      if (trackingData.ShipmentDetails.cust_id && trackingData.ShipmentDetails.cust_id.toString() !== userId.toString()) {
        console.warn(`SECURITY VIOLATION: Customer ID mismatch for user ${userId} accessing shipment ${shipmentId}`);
        return rejectWithValue("UNAUTHORIZED ACCESS: Customer verification failed. This shipment does not belong to your account.");
      }
      ...
      if (error?.response?.status === 401) {
        return rejectWithValue("Your session has expired. Please log in again to track your shipments.");
      }
      ...
      if (error?.response?.status === 404) {
        return rejectWithValue("Shipment not found or you don't have permission to access it.");
      }
